...
class TestSingleEdit:
    def test_basic_replacement(self, tmp_file: TmpFileFactory):
    ...
    def test_tuple_form(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        apply_edits(path, [("x", "y")])
        assert path.read_bytes() == b"y\n"

    def test_dict_form(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        apply_edits(path, [{"old": "x", "new": "y"}])
        assert path.read_bytes() == b"y\n"

    def test_deletion(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "before\nDELETE_ME\nafter\n")
        apply_edits(path, [Edit("DELETE_ME\n", "")])
        assert path.read_bytes() == b"before\nafter\n"

    def test_insertion(self, tmp_file: TmpFileFactory):
...
class TestMultipleEdits:
    def test_disjoint_edits(self, tmp_file: TmpFileFactory):
    ...
    def test_overlapping_edits_rejected(self, tmp_file: TmpFileFactory):
    ...
    def test_adjacent_non_overlapping_ok(self, tmp_file: TmpFileFactory):
    ...
    def test_ten_edits(self, tmp_file: TmpFileFactory):
...
class TestErrors:
    def test_empty_old_rejected(self, tmp_file: TmpFileFactory):
    ...
    def test_text_not_found(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "stuff\n")
    ...
    def test_ambiguous_match(self, tmp_file: TmpFileFactory):
    ...
    def test_overlapping_occurrences_are_ambiguous(self, tmp_file: TmpFileFactory):
    ...
    def test_exact_unique_match_ignores_fuzzy_equivalent_occurrences(
        self, tmp_file: TmpFileFactory
    ):
    ...
    def test_fuzzy_equivalent_occurrences_are_ambiguous_for_fuzzy_match(
        self, tmp_file: TmpFileFactory
    ):
        path = tmp_file("a.txt", "print(\u201chello\u201d)\nprint(\u201ehello\u201f)\n")
        with pytest.raises(AmbiguousMatchError) as exc:
    ...
    def test_no_changes(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "stuff\n")
    ...
    def test_noop_edit_in_multi_edit_write_rejected(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "alpha\nbeta\n")
    ...
    def test_fuzzy_equivalent_noop_write_rejected(self, tmp_file: TmpFileFactory):
    ...
    def test_empty_edits_list(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(ValueError):
            apply_edits(path, [])

    def test_invalid_edit_type(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TypeError, match="expected Edit"):
            apply_edits(path, [42])  # type: ignore[list-item]

    def test_invalid_tuple_length(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TypeError, match="tuple must be"):
            apply_edits(path, [("only-one",)])

    def test_invalid_tuple_item_types(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TypeError, match="tuple items must be str"):
            apply_edits(path, [("a", 42)])

    def test_mapping_missing_keys(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TypeError, match="mapping must have"):
            apply_edits(path, [{"old": "x"}])

    def test_mapping_non_str_values(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TypeError, match="must be str"):
            apply_edits(path, [{"old": "x", "new": 42}])

    def test_failed_edit_does_not_modify_file(
        self, tmp_file: TmpFileFactory, read_bytes: ReadBytes
    ):
...
class TestLineEndings:
    def test_lf_preserved(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "a\nb\nc\n")
        apply_edits(path, [Edit("b", "B")])
        assert read_bytes(path) == b"a\nB\nc\n"

    def test_crlf_preserved(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "a\r\nb\r\nc\r\n")
        apply_edits(path, [Edit("b", "B")])
        assert read_bytes(path) == b"a\r\nB\r\nc\r\n"

    def test_crlf_with_lf_in_edit_text(
        self, tmp_file: TmpFileFactory, read_bytes: ReadBytes
    ):
    ...
    def test_no_trailing_newline(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
...
class TestBom:
    def test_bom_preserved(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        bom = b"\xef\xbb\xbf"
        path = tmp_file("a.txt", "")
        # Write raw with BOM.
    ...
    def test_bom_not_required_in_old_text(self, tmp_file: TmpFileFactory):
        bom = b"\xef\xbb\xbf"
...
class TestFuzzyMatching:
    def test_smart_quotes(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "print(\u201chello\u201d)\n")
        result = apply_edits(path, [Edit('print("hello")', 'print("HELLO")')])
    ...
    def test_em_dash(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "before\u2014after\n")
        apply_edits(path, [Edit("before-after", "BEFORE-AFTER")])
        assert read_bytes(path) == b"BEFORE-AFTER\n"

    def test_nbsp(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "hello\u00a0world\n")
        apply_edits(path, [Edit("hello world", "HELLO WORLD")])
        assert read_bytes(path) == b"HELLO WORLD\n"

    def test_one_fuzzy_normalizes_whole_file(
        self, tmp_file: TmpFileFactory, read_bytes: ReadBytes
    ):
    ...
    def test_fuzzy_match_inserts_new_text_verbatim(self, tmp_file: TmpFileFactory):
        path = tmp_file(
            "a.txt",
            "before\u2014after\nother \u201cquote\u201d\n",
        )
    ...
    def test_fuzzy_flag_false_when_all_exact(self, tmp_file: TmpFileFactory):
...
class TestDryRun:
    ...
    def test_preview_alias(self, tmp_file: TmpFileFactory, read_bytes: ReadBytes):
        path = tmp_file("a.txt", "x\n")
        result = preview_edits(path, [Edit("x", "y")])
        assert result.written is False
        assert read_bytes(path) == b"x\n"

    def test_dry_run_no_changes_returns_no_change_result(
        self, tmp_file: TmpFileFactory, read_bytes: ReadBytes
    ):
    ...
    def test_dry_run_fuzzy_equivalent_noop_returns_no_change_result(
        self, tmp_file: TmpFileFactory
    ):
        path = tmp_file("a.txt", "before\u2014after\n")
        result = apply_edits(
...
class TestAtomicWrite:
    def test_no_temp_file_left_behind_on_success(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "hello\n")
        apply_edits(path, [Edit("hello", "world")])
        siblings = list(path.parent.iterdir())
        assert siblings == [path]

    def test_file_mode_preserved(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "hello\n")
        os.chmod(path, 0o640)
        apply_edits(path, [Edit("hello", "world")])
    ...
    def test_temp_file_removed_when_replace_fails(
        self, tmp_file: TmpFileFactory, monkeypatch: pytest.MonkeyPatch
    ):
        path = tmp_file("a.txt", "hello\n")
        ...
        monkeypatch.setattr("patch_tool.apply.os.replace", fail_replace)
    ...
    def test_parent_directory_fsynced_after_replace(
        self, tmp_file: TmpFileFactory, monkeypatch: pytest.MonkeyPatch
    ):
        path = tmp_file("a.txt", "hello\n")
        ...
        def record_fsync_directory(parent: Path) -> None:
            synced.append(parent)

        monkeypatch.setattr("patch_tool.apply._fsync_directory", record_fsync_directory)

        apply_edits(path, [Edit("hello", "world")])

        assert synced == [path.parent]

    def test_replaces_in_place(self, tmp_file: TmpFileFactory):
...
class TestPathHandling:
    def test_str_path_accepted(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
    ...
    def test_returned_path_is_resolved(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
...
class TestErrorMessages:
    def test_single_edit_uses_singular_phrasing(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "x\n")
        with pytest.raises(TextNotFoundError, match="the text"):
            apply_edits(path, [Edit("missing", "y")])

    def test_multi_edit_indexes_in_messages(self, tmp_file: TmpFileFactory):
        path = tmp_file("a.txt", "alpha\n")
